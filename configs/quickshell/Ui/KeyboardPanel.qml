...
//
// NOT WIRED IN YET (Phase 1: base-layer atom only). This still assumes
// upstream's click-driven popout-coordinator API on `bar`
// (clickTargets/requestPopout/releasePopout/activePopout), which this
// repo's Bar.qml doesn't implement — it only has the hover-based
// hoverPanelEnter/hoverPanelExit pair that HoverPanel.qml uses. Whichever
// later phase actually instantiates this needs to either add that
// coordinator API to Bar.qml or adapt this file's popout-switching/
// outside-click-forwarding logic to the hover model first.
//
...
PanelWindow {
  ...
  // upstream: Border.surfaceSpec("popups", "border", Color.popups.border, ...)
  // reads a [popups] border override from shell.toml; this repo has no such
  // override layer (see Commons/Color.qml's note), so it always resolves to
  // the flat fallback color/width below regardless.
  property var borderSpec: Border.flat(Color.popups.border, Math.max(1, Style.space(2)))
