...
// omarchy-reminder -> reminder (both this repo's own ports, symlinked via
// scripts/link.sh); Border.surfaceSpec -> Border.flat, Style.font.menuFamily
// -> Style.font.family (same adaptation as every other menu-styled overlay
// this session — Clipboard/AppSearch/Emojis/image-picker);
// Util.editsFilter/editedFilter (no local equivalent) -> explicit
// Key_Backspace + printable-character handling, same as those.
Item {
  id: root
  ...
  readonly property string notifyScript: Quickshell.env("HOME") + "/.local/bin/notification-send"
