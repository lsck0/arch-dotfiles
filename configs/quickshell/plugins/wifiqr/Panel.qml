...
// Adapted from omarchy-shell almost verbatim -- centered Wi-Fi share
// overlay: no card, just the QR code floating on a heavy scrim. Esc or the
// scrim dismiss it.
//
// `omarchy-network-qr`/`omarchy-network-password` -> this repo's own
// configs/quickshell/scripts/network-qr.sh / network-password.sh (verbatim
// ports, symlinked into ~/.local/bin by configs/quickshell/link.sh).
// `keepLoaded: true` on the manifest --
// see plugins/speedtest/Panel.qml's header comment for why any on-demand
// panel whose close() needs a Process to shut down gracefully needs this,
// not just the always-on ones.
//
// Standalone panel plugin: each summon regenerates the code via
// network-qr, which emits the interface, security, and SSID it shared
// ahead of the module matrix -- so a bare summon self-detects the
