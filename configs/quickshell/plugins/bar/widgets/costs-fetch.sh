#!/usr/bin/env bash
# Multi-cloud cost snapshot for Costs.qml.
set -euo pipefail
...
if [[ -f "$CREDS_DIR/hetzner_token" ]]; then
  ...
  # Hetzner Cloud has no native "current spend" endpoint; this sums per-server hourly price * hours-running-this-month as an estimate, not exact billing.
...
if [[ -f "$CREDS_DIR/cloudflare_token" ]]; then
  # Cloudflare's billing API needs an Enterprise plan; free/pro accounts have no programmatic spend endpoint.
  cloudflare_cost="null"
fi

gcp_cost="null"
if [[ -f "$CREDS_DIR/gcp_billing_sa.json" ]]; then
  # Needs a service account with roles/billing.viewer and a configured BigQuery billing export — not just the key file.
  gcp_cost="null"
fi
