...
BarWidget {
  id: root
  // Matches the registry id exactly (unlike every other widget's short
  // internal name, e.g. Clock's "clock") — persistTrayState() below needs
  // this to find its own shell.json layout entry via updateEntryInline(),
  // and Tray doesn't use moduleName for hover-panel tracking at all (it
  // manages managePopupOpen/trayMenuOpen itself via PopupCard, not
  // bar.activePanel), so there's no other use this could conflict with.
  moduleName: "bar.tray"

  // Always open — the drawer used to only reveal on hover, matching
  // Waybar's tray-expander default, but a systemtray that hides itself
  // behind an extra hover step just to see what's running was reported as
  // more annoying than useful. Kept as a real property (not a literal
  // `true` in revealProgress below) so a future per-user preference has
  // somewhere to plug in without touching the reveal math again.
  ...
  // Renders a tray icon, tinting it toward the bar foreground so every icon
  // blends with the current theme.  Symbolic icons get full recoloring (1.0);
  // non-symbolic icons use partial colorization (0.85) to retain a hint of
  // their original color identity while still fitting in.
